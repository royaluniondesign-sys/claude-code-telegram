...
Contains:
  agentic_text              — main text handler with smart routing (queued per chat)
  _agentic_text_impl        — actual routing implementation
...
Module-level guardrail state (exported for tests):
  _CHAT_LOCKS, _CHAT_PENDING, _CHAT_TASKS, _CHAT_LAST_SQUAD
  _squad_guardrail_decision, _classify_mission_mode
"""
...
def _env_int(name: str, default: int, minimum: int, maximum: int) -> int:
    """Read an int from env with bounds clamping."""
...
def _env_float(name: str, default: float, minimum: float, maximum: float) -> float:
    """Read a float from env with bounds clamping."""
    raw = os.getenv(name, "").strip()
    if not raw:
        return default
    try:
...
def _classify_mission_mode(text: str, has_active_mission: bool) -> tuple[str, str]:
    ...
    Returns (mode, normalized_text) where mode is one of:
      "new"      — explicit new mission request
      "continue" — explicit or implicit continuation
...
def _squad_guardrail_decision(
    ...
    """Return (allow_squad, reason) under cost/flow guardrails.

    Returns True (allow) when squad should proceed, False (skip) with reason
    string when the guardrail fires. Reasons: short_prompt, cooldown,
    brain_rate_limited:<name>, brain_pressure:<name>:<pct>.
    """
