...
class MessageOrchestrator(ZeroTokenMixin, FleetCommandsMixin):
    ...
    async def _handle_conductor_task(
        ...
        The same cognitive loop (ADENTRO) that AURA uses for self-improvement
        now applies to Ricardo's complex external tasks:
        - L0: meta_context already injected by conductor._create_plan()
        - L1: research/analysis brain (gemini/ollama)
        - L2: synthesis/generation brain
        - L3: final output (haiku/sonnet)
        - Outcome written to conductor_log.md (unified learning)
        """
        ...
        try:
            ...
            if result.is_error or not output:
                ...
                write_external_outcome(
                    task=task[:80], route="complex",
                    success=False, duration_s=duration_s,
            ...
            for i, chunk in enumerate(chunks):
                prefix = f"<b>🧠 Conductor</b> ({duration_s}s · {result.steps_completed}✓)\n\n" if i == 0 else ""
                await update.message.reply_text(prefix + chunk, parse_mode="HTML")

            write_external_outcome(
                task=task[:80], route="complex",
                success=True, duration_s=duration_s,
            ...
            write_external_outcome(
                task=task[:80], route="complex",
                success=False, duration_s=240.0,
            ...
            write_external_outcome(
                task=task[:80], route="complex",
                success=False, duration_s=round(_time.time() - t_start, 1),
    ...
    async def agentic_text(
        ...
        if router:
            ...
            # Classify task complexity before routing to brain.
            # Complex tasks (reports, analysis, multi-step) → conductor (Ruta B)
            # Simple tasks (questions, quick queries) → direct brain (Ruta A)
            try:
            ...
            if routed_brain != "zero-token":
                ...
                try:
                    ...
                    write_external_outcome(
                        ...
                        route="simple",
                        success=True,
                        duration_s=round(__import__('time').time() - _t_start, 1),
