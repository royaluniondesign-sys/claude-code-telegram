...
Telegram bot providing remote access to Codex. Python 3.10+, built with Poetry, using `python-telegram-bot` for Telegram and `Codex-agent-sdk` for Codex integration.
...
`ClaudeIntegration` (facade in `src/Codex/facade.py`) wraps `ClaudeSDKManager` (`src/Codex/sdk_integration.py`), which uses `Codex-agent-sdk` with `ClaudeSDKClient` for async streaming. Session IDs come from Codex's `ResultMessage`, not generated locally.
